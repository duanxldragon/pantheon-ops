...
后端错误 key 前缀：`cmdbhost.*`、`cmdbgroup.*`。

| Key | 说明 |
| :--- | :--- |
| `cmdbhost.not_found` | 主机不存在 |
| `cmdbhost.ip_exists` | IP 已存在 |
| `cmdbhost.ssh_connect_failed` | SSH 连接失败 |
| `cmdbhost.ssh_auth_failed` | SSH 认证失败 |
| `cmdbhost.collect_failed` | 配置采集执行失败 |
| `cmdbhost.unsupported_os` | 不支持的操作系统 |
| `cmdbhost.invalid_label` | 标签格式无效 |
| `cmdbgroup.not_found` | 分组不存在 |
| `cmdbgroup.invalid_conditions` | 过滤条件表达式无效 |
| `cmdblabel.key_exists` | 标签键已存在 |
| `cmdblabel.invalid` | 标签规范配置无效 |
| `cmdblabel.in_use` | 标签已被主机或分组引用 |
...
| 权限点 | 控制粒度 |
| :--- | :--- |
| `business:cmdb:host:list` | 页面可见 + 接口 |
...
| 权限点 | 控制粒度 |
| :--- | :--- |
| `business:cmdb:group:list` | 页面可见 + 接口 |
...
| `business:cmdb:label:list` | 标签规范页面 |
...
| :--- | :--- | :--- | :--- | :--- |
| `operations` | `/operations` | `operations.menu` | — | `D` |
| `operations.cmdb` | `/operations/cmdb` | `operations.cmdb.menu` | — | `M` |
| `operations.cmdb.host` | `/operations/cmdb/host` | `operations.cmdb.host.menu` | `business/cmdb/host/CmdbHostList` | `C` |
| `operations.cmdb.group` | `/operations/cmdb/group` | `operations.cmdb.group.menu` | `business/cmdb/group/CmdbGroupList` | `C` |
| `operations.cmdb.label` | `/operations/cmdb/label` | `operations.cmdb.label.menu` | `business/cmdb/label/CmdbLabelSchemaList` | `C` |
...
- **删除保护**：当标签键已出现在 Host 标签或 Group 条件中时，删除失败并返回 `cmdblabel.in_use`。
- **主机/分组表单联动**：主机标签和分组条件的 key 使用启用状态的 LabelSchema 作为候选；value 根据 key 联动展示 `options`，`free` 标签仍允许手动输入。

### 9.7 页面状态规范

所有页面必须覆盖：加载中、空数据、无权限（403）、服务端错误、提交中（表单）、删除确认弹窗。
...
后端错误 key 前缀：`cmdbhost.*`、`cmdbgroup.*`（见 6.3 节），与前端 i18n 分离。
...
| 权限 | `business:cmdb:host:*`（7 项）、`business:cmdb:group:*`（5 项） | 见第 7 节 |
...
- 无权限用户不可见运维平台菜单。
- 有 `list` 无 `create` 用户，新增按钮不可见且接口 403。
- 无权限用户调用采集接口返回 403。
