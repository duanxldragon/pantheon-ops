...
- Platform shell and cross-domain UI checks run through `npm run test:smoke:platform`.
- System-domain checks run through `npm run test:smoke:system`.
- Business checks run through `npm run test:smoke:business`.
...
- Browser smoke commands must run through `scripts/run-smoke-suite.mjs` so the started Vite server and Playwright `baseURL` stay aligned on `PANTHEON_WEB_BASE_URL`.
...
- Do not hard-code `--proxy-target http://127.0.0.1:8080` in smoke npm scripts. Set `PANTHEON_API_PROXY_TARGET` in the environment so suites can follow the active backend instance.
...
- Generated business runtime smoke still lives under `system/governance/` paths; script ownership is the current normalization boundary.
