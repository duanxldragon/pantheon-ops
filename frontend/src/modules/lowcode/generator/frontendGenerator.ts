...
export class FrontendGenerator {
  ...
  generateModuleIndex(): string {
    ...
    const generateDetailRoute =
      generateNavigation && (pageActions.includes('view') || pageActions.includes('detail'));
...
${this.generateRowActionItems()}
  ...
  private generateListActionImports(): string {
    ...
    if (this.hasPageAction('delete') && this.isListBatchActionsEnabled()) {
...
${searchableFields
  ...
  private generateWorkbenchHeaderActions(
    ...
    const listHeaderActions = `        <ListHeaderActions
          utility={
            <>
${utilityActions.join('\n')}
            </>
    ...
    const functions = relationFields
      .map((field) => {
        const relation = (this.schema.relations ?? []).find(
          (item) => item.localField === field.name,
        );
        const functionName = this.toPascalCase(field.name);
