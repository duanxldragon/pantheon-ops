...
import (
	...
	auditmod "pantheon-base/modules/system/audit"
	settingmod "pantheon-base/modules/system/config/setting"
	rolemod "pantheon-base/modules/system/iam/role"
...
func TestRuntime_ReloadsMFASettingAfterSettingUpdate(t *testing.T) {
	db := testmysql.Open(t)
...
func TestRuntime_VerifyPasswordForOperationBindsSession(t *testing.T) {
	...
	claims, err := authtoken.ParseOperationToken(token, rdb)
	if err != nil {
...
func TestRuntime_CreateSessionRevokesOlderActiveSessionsByConfiguredLimit(t *testing.T) {
	...
	pair, err := s.CreateSession(testUser.ID, nil, "127.0.0.1", "Mozilla/5.0")
	if err != nil {
...
func TestRuntime_ExportLoginLogsCapsRows(t *testing.T) {
	...
	now := time.Now()
	for i := 0; i < maxLoginLogExportRows+1; i++ {
		rows = append(rows, SystemLogLogin{
			Username:  "bulk",
			Ipaddr:    "127.0.0.1",
			Browser:   "Chrome",
			Os:        "Windows",
			Status:    1,
			Msg:       "auth.loginSuccess",
			LoginTime: now.Add(time.Duration(i) * time.Second),
		})
	}
	...
	exported, err := s.ExportLoginLogs(&LoginLogQuery{})
	if err != nil {
		t.Fatalf("export login logs: %v", err)
	}
	if len(exported.Rows) != maxLoginLogExportRows {
		t.Fatalf("expected export cap %d, got %d", maxLoginLogExportRows, len(exported.Rows))
	}
}
