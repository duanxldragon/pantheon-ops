...
ALTER TABLE `biz_deploy_task`
  ADD COLUMN `execution_snapshot` JSON,
  ADD COLUMN `target_snapshot` JSON,
  ADD COLUMN `start_request_key` VARCHAR(128) NOT NULL DEFAULT '';

ALTER TABLE `biz_deploy_task_host`
  ADD COLUMN `ssh_port` INT NOT NULL DEFAULT 22,
  ADD COLUMN `business_scope_id` BIGINT UNSIGNED NOT NULL DEFAULT 0,
  ADD COLUMN `report_key` VARCHAR(128) NOT NULL DEFAULT '',
  ADD COLUMN `resolved_at` DATETIME(3) DEFAULT NULL;
